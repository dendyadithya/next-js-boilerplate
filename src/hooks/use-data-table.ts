...
interface UseDataTableProps<TData>
  ...
  initialState?: Omit<Partial<TableState>, 'sorting'> & {
    ...
    /**
     * The function to determine if a row can be expanded.
     * @default undefined
     * @type (row: Row<TData>) => boolean
     * @example
     * ```ts
     * const isExpanded = (row: Row<TData>) => row.original.isExpanded;
     * ```
     */
    isExpanded?: boolean
  }
...
export function useDataTable<TData>({
  ...
  const debouncedSetFilterValues = useDebounceCallback(setFilterValues, debounceMs)
  ...
  const table = useReactTable({
    ...
    getRowCanExpand: () => initialState?.isExpanded ?? false,
